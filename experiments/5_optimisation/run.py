...
def objective(trial):
    ...
    plate_off = trial.suggest_uniform('plate_offset', -0.25, 5.0)  # Plate offset between -0.25 and 5.0 cm
    cone_off = trial.suggest_uniform('cone_offset', -0.25, 0.50)  # Cone offset between -0.25 and 0.50 cm
    ...
    # Return the objective value (impingment rate on back of plate)
    return row['plate_back_imping_rate']  


study = optuna.create_study(direction='maximize')  # We want to maximize the impingment rate
